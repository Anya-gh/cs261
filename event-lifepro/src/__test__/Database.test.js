...
test("Database", async () => {
    ...
    expect(k1).toEqual({keyID: 1111, eventID: 111, keyObject: {attendeeKey: "attK 1", hostKey: "hK 1"}});
    const k2 = await prisma.key.findUnique({where: {keyID: 1112}});
    expect(k2).toEqual({keyID: 1112, eventID: 112, keyObject: {attendeeKey: "attK 2", hostKey: "hK 2"}});
    const k3 = await prisma.key.findUnique({where: {keyID: 1113}});
    expect(k3).toEqual({keyID: 1113, eventID: 113, keyObject: {attendeeKey: "attK 3", hostKey: "hK 3"}});
    const u1 = await prisma.user.findUnique({where: {userID: 11111}});
    expect(u1).toEqual({userID: 11111, eventID: 111, userObject: {name: "User 1"}});
    const u2 = await prisma.user.findUnique({where: {userID: 11112}});
    expect(u2).toEqual({userID: 11112, eventID: 111, userObject: {name: "User 2"}});
    const u3 = await prisma.user.findUnique({where: {userID: 11113}});
    expect(u3).toEqual({userID: 11113, eventID: 112, userObject: {name: "User 3"}});
    const r1 = await prisma.response.findUnique({where: {responseID: 111111}});
    expect(r1).toEqual({responseID: 111111, eventID: 111, userID: 11111, responseObject: {userID: 11111, time: "1210", answers: ["It is good"]}});
    const r2 = await prisma.response.findUnique({where: {responseID: 111112}});
    expect(r2).toEqual({responseID: 111112, eventID: 111, userID: 11112, responseObject: {userID: 11112, time: "1150", answers: ["It is boring"]}});
    const r3 = await prisma.response.findUnique({where: {responseID: 111113}});
    expect(r3).toEqual({responseID: 111113, eventID: 112, userID: 11113, responseObject: {userID: 11113, time: "0930", answers: ["I like it so far"]}});
    const r4 = await prisma.response.findUnique({where: {responseID: 111114}});
    expect(r4).toEqual({responseID: 111114, eventID: 112, userID: 11113, responseObject: {userID: 11113, time: "0948", answers: ["This part is not interesting"]}});
    } catch(e) {
        throw e;
    } finally {
        prisma.$disconnect();
